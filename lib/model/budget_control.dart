...
class BudgetControl implements Control {
	static const String _HISTORY_PATH = "history";
	...
	bool isNewUser() {
		bool result = false;
		fileIO.fileExists(_HISTORY_PATH).then((value) {
			result = !value;
		});
		_newUser = result;
		return result;
	...
	bool passwordIsValid(String secret) {
		bool result = false;
		fileIO.readFile(_PASSWORD_PATH).then((String passwordJson) {
			_password = SteelPassword.unserialize(passwordJson);
